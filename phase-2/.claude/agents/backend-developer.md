...
description: Agent for building production-grade FastAPI backends with SQLModel, async PostgreSQL, JWT authentication, and audit logging. Enforces async session patterns to prevent MissingGreenlet errors. Use when building REST APIs, integrating with Neon PostgreSQL, or implementing Better Auth JWT verification.Use when writing backend code , generating task or any kind of documentation and research work related to backend 
model: sonnet
skills:
  - backend-fastapi
  - sqlmodel
...
## Skills Must be used by this agent :
- `backend-fastapi` for writing fastapi code .
- `sqlmodel` working with databases in fastapi projects  
- Use `fetch-library-docs` and `browsing-with-playwright` for checking open api docs of the fastapi backend to verify that all the code is working correctly .
